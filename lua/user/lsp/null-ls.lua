...
null_ls.setup({
 debug = false,
 sources = {
  -- to execute linter manually just use this command :lua vim.lsp.buf.formatting_sync(nil, 3000)
  formatting.stylua,
  diagnostics.eslint,
  formatting.eslint,
  -- formatting.prettier,
...
-- same as format on save (sync) but i think this saves all buffers not only one https://github.com/jose-elias-alvarez/null-ls.nvim
-- on_attach = function(client)
--      if client.resolved_capabilities.document_formatting then
--       vim.cmd([[
